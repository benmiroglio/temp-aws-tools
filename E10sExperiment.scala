...
object E10sExperimentView {
  ...
  private def run(opts: Opts, messages: RDD[Message]) {
    ...
    messages
      .flatMap { message =>
        ...
        val cohortField = settings \ "e10sCohort"
        val JString(cohort) = cohortField match {
          // dummy json for typing
          case JNothing => parse(""" {"dummy":"missing"} """) \ "dummy"
          case _ => cohortField
        }
        ...
        val e10sEnabledField = settings \ "e10sEnabled"
        val JBool(e10sEnabled) = e10sEnabledField match {
          // dummy json for typing
          case JNothing => parse(""" {"dummy":false} """) \ "dummy"
          case _ => e10sEnabledField
        }

        val pingShouldBeExcluded = (cohort ==  "missing") || // missing fields
          (cohort == "test" && !e10sEnabled) || // should have e10s enabled, but doesn't
  ...
  private def buildSchema: Schema = {
    SchemaBuilder
      ...
      .name("submission").`type`().stringType().noDefault()
  ...
  private def buildRecord(fields: Map[String, Any], schema: Schema): Option[GenericRecord] = {
    ...
    val root = new GenericRecordBuilder(schema)
      ...
      .set("submission", fields.getOrElse("submission", "{}"))
